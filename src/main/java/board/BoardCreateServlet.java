	...
	public class BoardCreateServlet extends HttpServlet {
		...
		protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
	        ...
	        if (multi.getParameter("ref").equals("null")) {
	        	ref = boardDao.boardRef();
	        }else {
	        	ref = Integer.parseInt(multi.getParameter("ref"));
	        }
	        if (multi.getParameter("ref_step").equals("null")) {
	        	ref_step = 0;
	        }else {
	        	ref_step = Integer.parseInt(multi.getParameter("ref_step"))+1;
	        }
	        if (multi.getParameter("ref_level").equals("null")) {
	        	ref_level = 0;
	        }else {
	        ...
	        String password="";
	       try {
				password = encrypt.aesEncode(multi.getParameter("password"));
